...
FOR SELECT 
USING (
  receiver_cf_handle IN (
    SELECT cf_handle FROM codeforces_users 
    WHERE handle = (
      SELECT raw_user_meta_data->>'codeforces_handle' 
      FROM auth.users 
      WHERE id = auth.uid()
    )
  )
...
CREATE POLICY "Users can update duel requests sent to them" 
ON public.duel_requests 
FOR UPDATE 
USING (
  receiver_cf_handle IN (
